...
public class GenericOpenAiProvider implements LlmProvider {
    ...
    public String chat(ChatRequest request) {
        ...
        String url = baseUrl.endsWith("/v1") ? baseUrl + "/chat/completions"
                : baseUrl.endsWith("/") ? baseUrl + "v1/chat/completions"
        ...
        try (Response response = client.newCall(httpRequest).execute()) {
            ...
            if (!response.isSuccessful()) {
                log.error("Generic OpenAI API error: status={}, body={}", response.code(), responseBody);
                throw new BusinessException(500, buildErrorMsg("GenericOpenAI", response.code(), responseBody));
            }
            JSONObject json = JSON.parseObject(responseBody);
            return json.getJSONArray("choices").getJSONObject(0)
                    .getJSONObject("message").getString("content");
        } catch (IOException e) {
            log.error("Generic OpenAI API call failed", e);
